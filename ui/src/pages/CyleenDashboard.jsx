...
function CyleenDashboard() {
  ...
  if (loading) {
    return (
      <div className="dashboard">
        <div className="loading">Loading Cyleen dashboard...</div>
      </div>
    )
  }
  ...
  const summaryAlerts = [
    alerts.water_level_low && '🔴 Ultrasonic distance is outside the safe range',
    alerts.tds_alert && '💧 TDS is above target',
    alerts.ammonia_alert && '☠️ Ammonia is over the warning level',
  ].filter(Boolean)

  return (
    <div className="dashboard">
      <header className="dashboard-header">
        <h1>🦞 Cyleen Dashboard</h1>
        <p className="timestamp">Focus: IR, TDS, and ammonia</p>
      ...
      <div className="sensors-grid">
        ...
        <SensorCard
          ...
          alertMessage={alerts.tds_alert ? 'TDS is above target.' : null}
        ...
        <SensorCard
          ...
          alertMessage={alerts.ammonia_alert ? 'Ammonia is over the warning level.' : null}
          threshold={thresholds ? `${thresholds.AMMONIA_ALERT} ADC` : 'N/A'}
        />
      </div>

      <div className="alert-summary">
        <h3>⚡ Cyleen Summary</h3>
        <div className="alerts-list">
          {summaryAlerts.length ? summaryAlerts.map((item) => (
            <div className="alert-item" key={item}>{item}</div>
          )) : <div className="no-alerts">✅ IR, TDS, and ammonia look clear</div>}
        </div>
      </div>
    </div>
  )
...
export default CyleenDashboard
